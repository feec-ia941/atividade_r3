...
namespace WorldServerLibrary
{
    public class WorldServer:IDisposable
    {
        ...
        public WorldServer(String hostName, String port)
        {                                   
            Int32 portInt = 0;
        ...
        /// <param name="port">The Server Host Port</param>
        /// <exception cref="WorldServerInvalidArgument">Throws this exception if some argument is invalid</exception>
        public WorldServer(String hostName, Int32 port)
        {
            if (!String.IsNullOrWhiteSpace(hostName))
            {
                this.hostName = hostName.Trim();
        ...
        public List<Thing> SendGetCreatureState(string creatureId)
        {
            ...
            try
            {
				...
				firtCaracter = response.Substring(0, 1).ToUpper();

				// Parse the response
				if(firtCaracter == "C" || firtCaracter == " ")                
		...
		public void thriveWorld(){
			Thread thread = new Thread (new ThreadStart(generationThings));
			thread.Start ();
		}

		int countFood = 0;
		public void generationThings()
		{
			while (clientSocket != null) {
				...
				try {
					...
					for (int i = 0; i <= 1; i++) {
						Random r = new Random();
						int count = r.Next(1,2);
						for (int j = 0; j < count; j++) {
							NewFood(i,r.Next(150,500),r.Next(150,500));
						}
					}
						countFood = 0 ; 
					}
					...
					int count2 = r2.Next(1,5);

					for (int i = 0; i < count2; i++) {
						int color = r2.Next(0,6);
						NewJewel(color,r2.Next(150,500),r2.Next(150,500));
					}
